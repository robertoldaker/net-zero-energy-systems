...
pushd ASP_BUILD
"C:\Program Files\7-Zip\7z" a ..\SmartEnergyLabDataApi.zip .
popd
if errorlevel 1 goto error
"c:\Program files\PuTTY\psftp.exe" -b psftp.txt -pw speedy -bc rob@blackbox.local
if errorlevel 1 goto error
rem install on server
"c:\Program files\PuTTY\plink.exe" -pw speedy rob@blackbox.local "~/installs/autoInstall.sh SmartEnergyLabDataApi"
pause
exit /b
:error
echo "Error building SmartEnergyLabDataApi"
pause