...
using static SmartEnergyLabDataApi.Models.TaskRunner.TaskState;
...
namespace SmartEnergyLabDataApi.Models {
    public class DatabaseBackup {
        ...
        public void Run(bool sFtp = true, bool restoreToStagingServer = false)
        {
            string fileName = "";
            try {
                fileName = create();
                ...
                if (restoreToStagingServer) {
                    this.restoreToStagingServer(fileName);
                }
            } finally {
                if (!string.IsNullOrEmpty(fileName)) {
                    var backupPath = Path.Combine(LOCAL_PATH, fileName);
        ...
        private void restoreToStagingServer(string filename)
        {
            ...
            }, LOCAL_PATH, new Dictionary<string, string>() { { "PGPASSWORD", Program.DB_PASSWORD } });
            if (exitCode != 0) {
        ...
        {
            _taskRunner?.Update(RunningState.Running, "Creating backup ...");
        ...
        private string getPgDumpArgs(string? fileName = null, bool verbose = false)
        {
            var fn = fileName != null ? $"-f {fileName}" : "";
            var ver = verbose ? "-v" : "";
            var args = $"{ver} -h {Program.DB_HOST} -p {Program.DB_PORT} -U {Program.DB_USER} {fn} -Fc {Program.DB_NAME}";
            return args;
        }
        ...
        private string getPgRestoreArgs(string host, string fileName)
        {
            var args = $"--clean -v --if-exists -h \"{host}\" -p {Program.DB_PORT} -U {Program.DB_USER} -d {Program.DB_NAME} \"{fileName}\"";
            return args;
        }
    }
}
