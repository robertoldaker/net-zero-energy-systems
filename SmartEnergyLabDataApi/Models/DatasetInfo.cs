...
namespace SmartEnergyLabDataApi.Models
{
    public class DatasetInfo {
        public DatasetInfo( DataAccess da, int datasetId) {
            ...
            GenParameterInfo = new DatasetData<GenParameter>(da, datasetId,m=>m.Id.ToString(),q1);
            var q2 = da.Session.QueryOver<GenCapacity>().OrderBy(m=>m.OrderIndex).Asc;
            GenCapacityInfo = new DatasetData<GenCapacity>(da, datasetId,m=>m.Id.ToString(),q2);
            var q3 = da.Session.QueryOver<PeakDemand>();
            PeakDemandInfo = new DatasetData<PeakDemand>(da, datasetId,m=>m.Id.ToString(),q3);
            var q4 = da.Session.QueryOver<MiscParams>();
            MiscParamsInfo = new DatasetData<MiscParams>(da,datasetId,m=>m.Id.ToString(),q4);
            var q5 = da.Session.QueryOver<Link>();
            LinkInfo = new DatasetData<Link>(da,datasetId,m=>m.Id.ToString(),q5);
        }
