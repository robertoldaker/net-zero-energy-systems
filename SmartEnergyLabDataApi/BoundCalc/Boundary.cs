...
namespace SmartEnergyLabDataApi.BoundCalc
{
    public class Boundary {
        ...
        public delegate void AllTripsProgressHandler(BoundCalcBoundaryTrips.BoundCalcBoundaryTrip trip, int percent);
        public event AllTripsProgressHandler AllTripsProgress;

        private BoundCalc _lf;
        private NodeBoundaryData _nbd;
        private BoundCalcBoundaryTrips _bts;

        public Boundary(BoundCalc lf) {
            _lf = lf;
        }
        ...
        public BoundCalcBoundaryTrips BoundaryTrips {
            get {
                return _bts;
            }
        }
        ...
        public BoundCalcBoundaryFlowResult? SetBound(string boundaryCode) {
            double[] ivang=null;
            int i, p;
            double[] mism;
            double kgin, kdin;
            ...
            BoundCalcBoundaryFlowResult? bfr = null;
            ...
            if ( !_lf.NetCheck()) {
                _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Fail,"");
                return null;
            }

            _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass,"");
            ...
            _lf.StageResults.StageResult(sr,BoundCalcStageResultEnum.Pass,$"Zones in boundary = {_nbd.Count}");
            ...
            bfr = new BoundCalcBoundaryFlowResult(gin,din,gout,dout,_ia);

            _lf.StageResults.StageResult(sr,BoundCalcStageResultEnum.Pass,$"{_boundnm} base transfer = {_pfer:f1} IA = {_ia:f1}");
            ...
            if ( Math.Abs(mism[mi])<0.1 ) {
                ...
                _lf.StageResults.StageResult(sr,BoundCalcStageResultEnum.Pass,$"Max mismatch {mism[mi]:g1} at {node.Obj.Code}");
            } else {
                var node = _lf.Nodes.get(_lf.Nord.NodeId(mi));
                _lf.StageResults.StageResult(sr,BoundCalcStageResultEnum.Pass,$"Max mismatch {mism[mi]:f1} at {node.Obj.Code}");                
            }

            sr = _lf.StageResults.NewStage("Identity boundary circuits");

            _bts = new BoundCalcBoundaryTrips(_lf.Branches,_nbd);

            _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass, $"{_bts.LineNames.Count} boundary circuits with total cap {_bts.TotalCapacity:f0}");
        ...
        public int RunBoundaryOptimiser(CVang cvang) {
            ...
            if ( r1 == LPhdr.lpOptimum ) {
                ...
                _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Fail, "Planned transfer failed");
                return r1;
            }
            ...
            if ( r1 == LPhdr.lpOptimum ) {
                ...
                _lf.StageResults.StageResult(sr,BoundCalcStageResultEnum.Pass,$"Boundcap = {boundCap:f1} Control cost = {controlCost:f2}");
            } else if ( r1 == LPhdr.lpInfeasible ) {
                if ( _ctrllp.Slack(r2) < -0.1) {
                    _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Fail,$"Unresolvable constraint {_ctrllp.GetCname(r2)}");
                } else {
                    r1 = LPhdr.lpOptimum;
                }
            } else {
                _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Fail, "Unknown optimiser fail");
            }
            ...
            if ( r1!=LPhdr.lpOptimum || DIAGNOSE ) {
                ...
                if ( Math.Abs(mm) < 0.1 ) {
                    _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass, $"Max mismatch {mm:g1} at {nw.Obj.Code}");
                } else {
                    _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass, $"Max mismatch {mm:f1} at {nw.Obj.Code}");
                }
                _lf.Optimiser.ReportConstraints(BoundCalcStageResultEnum.Warn);
            ...
            int i=0;
            int cct=0;
            BoundCalcStageResult sr1;
            sr = _lf.StageResults.NewStage($"Optimise {_boundnm} all cct limits");
            do {
                iter++;
                i = 1;
                while( free[ord[i]] < BoundCalc.OVRLD && i<= MAXCPI ) {
                    cct = ord[i];
                ...
                if ( r1 == LPhdr.lpOptimum ) {
                    boundCap = _pfer + Math.Sign(_pfer)*xfer;
                    controlCost = _ctrllp.Slack(_lf.Optimiser.bounzc.Id) - _ctrllp.Objective();
                    _lf.StageResults.StageResult(sr1, BoundCalcStageResultEnum.Pass, $"Boundcap = {boundCap:f1} Control cost = {controlCost:f2}");
                    ...
                    _lf.StageResults.StageResult(sr1, BoundCalcStageResultEnum.Fail, "Unknown optimiser fail");
                } 
                ...
                if ( r1!=LPhdr.lpOptimum || DIAGNOSE ) {
                    ...
                    // Output diagnosis results
                    boundCap = _pfer + Math.Sign(_pfer)*xfer;
                    _lf.StageResults.StageResult(sr1, BoundCalcStageResultEnum.Warn, $"Boundcap = {boundCap:f1}");
                    _lf.Optimiser.ReportConstraints(BoundCalcStageResultEnum.Warn);
                    _lf.ReportOverloads(free, ord);
                }
            ...
            _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass, $"Control cost = {_ctrllp.Slack(_lf.Optimiser.bounzc.Id) - _ctrllp.Objective():f2}");
            ...
            nw = _lf.Nodes.get(_lf.Nord.NodeId(mi));
            if ( Math.Abs(mm) < 0.1 ) {
                _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass, $"Boundcap = {boundCap:f1} Max mismatch {mm:e1} = at {nw.Obj.Code}");
            } else {
                _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass, $"Boundcap = {boundCap:f1} Max mismatch {mm:f1} = at {nw.Obj.Code}");
            }
        ...
        public BoundCalcBoundaryFlowResult? RunAllBoundaryTrips(string boundaryName, out List<BoundCalcAllTripsResult> singleTrips, out List<BoundCalcAllTripsResult> doubleTrips) {
            double r;
            ...
            foreach( var trip in _bts.Trips) {
                ...
                if ( r == LPhdr.lpOptimum ) {
                    ...
                    var tripResult = new BoundCalcAllTripsResult() { Capacity = capacity, Trip = trip, LimCct = limCct, Ctrls = ctrls};
                    if ( trip.Type == BoundCalcBoundaryTripType.Single) {
        ...
        public BoundCalcBoundaryFlowResult? RunBoundaryTrip(string boundaryName, string tripName) {
            double r;
        ...
        public int RunBoundaryTrip(BoundCalcBoundaryTrips.BoundCalcBoundaryTrip trip) {
            int r1;
            ...
            if(!_lf.TripVectors(ccts,out tcva)) {
                if (_lf.Countac(tripList) > 0) {
                    _lf.StageResults.StageResult(sr,BoundCalcStageResultEnum.Fail,"Invalid trip - node disconnected?");
                    return r1;
                }
            }

            _lf.StageResults.StageResult(sr, BoundCalcStageResultEnum.Pass, $"{ccts.Length} circuits");
