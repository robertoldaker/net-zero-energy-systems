...
public class BoundCalcETYSLoader
{
    public enum BoundCalcLoadOptions {All,OnlyHighVoltageCircuits};

    private BoundCalcLoadOptions _loadOptions;
    ...
    public BoundCalcETYSLoader(BoundCalcLoadOptions loadOptions=BoundCalcLoadOptions.OnlyHighVoltageCircuits)
    {
    ...
    private bool updateNodeZones() {
        ...
        using( var da = new DataAccess() ) {
            ...
            var zones = da.BoundCalc.GetZones(dataset);
            //
    ...
    private void checkNodeDemands(List<NodeDemand> nodeDemands) {
        using( var da = new DataAccess()) {
            ...
            //
            var nodes = da.BoundCalc.GetNodes(ds);
            var ssNodes = da.BoundCalc.GetNodes(ssDs);
            //
            int numNotFound = 0;
            int numNotFoundSS = 0;
    ...
    private void assignNodeDemands(List<NodeDemand> nodeDemands) {
        using( var da = new DataAccess()) {
            ...
            //
            var nodes = da.BoundCalc.GetNodes(ds);
            var branches = da.BoundCalc.GetBranches(ds);
    ...
    private void addHVDCCtrls(DataAccess da) {
        ...
        var ssCtrls = da.BoundCalc.GetCtrls(ssDataset);
        var ssBranches = da.BoundCalc.GetBranches(ssDataset);

        // get current ones
        var dataset = getDataset(da, BASE_YEAR);
        var existingBranches = da.BoundCalc.GetBranches(dataset);
        var existingCtrls = da.BoundCalc.GetCtrls(dataset);
        var existingNodes = da.BoundCalc.GetNodes(dataset);
        var existingZones = da.BoundCalc.GetZones(dataset);

        // HVDC controls
        var ssCtrlsHVDC = ssCtrls.Where( m=>m.Type == BoundCalcCtrlType.HVDC).ToList();
    ...
    private void addQuadBoosters(DataAccess da) {
        ...
        //
        var existingCtrls = da.BoundCalc.GetCtrls(dataset);
        existingCtrls = existingCtrls.Where(m=>m.Type==BoundCalcCtrlType.QB).ToList();
    ...
    private void repairNodes() {
        using ( var da = new DataAccess() ) {
            var ssDataset = getSpreadsheetDataset(da);
            var nodes = da.BoundCalc.GetNodes(ssDataset);
            var zones = da.BoundCalc.GetZones(ssDataset);
            foreach( var node in nodes ) {
    ...
    private void updateNodes(List<string> nodeNames) {
        using ( var da = new DataAccess() ) {
            ...
            //
            var existingNodes = da.BoundCalc.GetNodes(dataset);
            var ssNodes = da.BoundCalc.GetNodes(ssDataset);
            var zones = da.BoundCalc.GetZones(dataset);
            if ( zones.Count==0 ) {
    ...
    private IList<BoundCalcZone> copyBoundariesAndZones(DataAccess da, Dataset ds, Dataset dataset) {
        ...
        foreach( var bz in boundaryZones ) {
            ...
            da.BoundCalc.Add(nbz);
        }
        return newZones;
    }

    private Dataset getDataset(DataAccess da, int year) {
        var ending = _loadOptions == BoundCalcLoadOptions.All ? " (all circuits)" : "";
        var datasetName = $"{DATASET_BASE_NAME} {year}{ending}";
        var dataset = da.Datasets.GetDataset(DatasetType.BoundCalc,datasetName);
        if ( dataset==null) {
            var root = da.Datasets.GetRootDataset(DatasetType.BoundCalc);
            dataset = new Dataset() { Type = DatasetType.BoundCalc, Parent = root, Name = datasetName };                
    ...
    private Dataset getSpreadsheetDataset(DataAccess da) {
        var name = "GB network";
        var ds = da.Datasets.GetDataset(DatasetType.BoundCalc,name);
        if ( ds==null) {
    ...
    private bool addCircuit(Circuit circuit) {
        if ( _loadOptions==BoundCalcLoadOptions.All ) {
            return true;
        } else if ( _loadOptions==BoundCalcLoadOptions.OnlyHighVoltageCircuits && circuit.IsHighVoltage) {
            return true;
        } else {
            return false;
        }
