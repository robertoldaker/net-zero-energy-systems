...
import { AllTripResult, BoundaryTrip, BoundaryTripResults, Branch, BranchType, Ctrl, CtrlResult, CtrlSetPoint, Dataset, DatasetData, DatasetType, GISData, GridSubstation, GridSubstationLocation, BoundCalcCtrlType, BoundCalcResults, NetworkData, Node, SetPointMode, GenerationModel, Generator, GeneratorType} from '../data/app.data';
...
export class SelectedMapItem {
    constructor(location:BoundCalcLocation | null = null,link: BoundCalcLink | null = null, locTab: MapItemLocationTab | null = null) {
        this.location = location
        this.link = link
        this.locTab = locTab
    }
    location: BoundCalcLocation | null = null
    link: BoundCalcLink | null = null
...
export class BoundCalcDataService {
    ...
    constructor(
        ...
        this.locMap = new Map<number,BoundCalcLocation>()
        this.linkMap = new Map<string,BoundCalcLink>()
    ...
    locMap: Map<number,BoundCalcLocation>
    linkMap: Map<string,BoundCalcLink>
    loadFlowResults: BoundCalcResults | undefined
    ...
    isBoundaryTrip(link: BoundCalcLink):boolean {
    ...
    isBoundaryLink(link: BoundCalcLink):boolean {
        return link.branches.find(m=>this.boundaryBranchIds.includes(m.id)) ? true : false
    }

    isBoundaryLimCct(link: BoundCalcLink):boolean {
    ...
    afterCalc(results: BoundCalcResults, saveTripResults:boolean = false) {
    ...
    public setSetPointMode(setPointMode: SetPointMode) {
        if ( setPointMode == SetPointMode.Auto) {
            ...
            if ( this.loadFlowResults == undefined) {
                throw "Unexpected undefined boundcalc results"
            }
    ...
    getLocation(locId: number):BoundCalcLocation | undefined {
        let loc = this.locationData.locations.find(m=>m.id==locId)
        return loc
    }
    ...
    searchLocations(str: string, maxResults: number):BoundCalcLocation[]  {
    ...
    searchMapData(str: string, maxResults: number):BoundCalcMapSearchItem[]  {
        let lowerStr = str.toLocaleLowerCase()
        let upperStr = str.toUpperCase()
        ...
        var searchResults:BoundCalcMapSearchItem[] = []
    ...
    search(str: string, maxResults: number):BoundCalcLocation[]  {
        let lowerStr = str.toLocaleLowerCase()
        let upperStr = str.toUpperCase()
        var searchResults = this.locationData.locations.
    ...
    */
    ...
    updateLocations(updateLocationData: UpdateLocationData) {
        //
        let updateLocs:BoundCalcLocation[] = []
        let deleteLocs:BoundCalcLocation[] = []
    ...
    updateLinks(updateLocationData: UpdateLocationData) {
        let updateLinks:BoundCalcLink[] = []
        let deleteLinks:BoundCalcLink[] = []
    ...
    private getDatasetData(typeName: string):DatasetData<any> {
        // Note always use this.networkData as boundcalcResults should be null here
        if ( typeName == "Node") {
    ...
    getFlowCapacityThreshold(type: BranchType, percentCapacity: number):PercentCapacityThreshold {
        if ( type!=BranchType.HVDC ) {
            if (percentCapacity>BoundCalcDataService.CriticalFlowThreshold ) {
                return PercentCapacityThreshold.Critical
            } else if ( percentCapacity>BoundCalcDataService.WarningFlowThreshold) {
    ...
    ResultsLoaded:EventEmitter<BoundCalcResults> = new EventEmitter<BoundCalcResults>()
...
export class UpdateLocationData {
    ...
    updateLocations: BoundCalcLocation[] = []
    deleteLocations: BoundCalcLocation[] = []
    updateLinks: BoundCalcLink[] = []
    deleteLinks: BoundCalcLink[] = []
    clearBeforeUpdate: boolean = false
}

export class BoundCalcLocation {
    ...
    update(nodes: Node[], ctrls: Ctrl[]):boolean {
        ...
        let isQB = ctrls.find(m=>m.node1LocationId === this._gsl.id && m.type == BoundCalcCtrlType.QB)!==undefined
...
export class BoundCalcMapSearchItem {
    loc: BoundCalcLocation | undefined
    link: BoundCalcLink | undefined
}
