...
import { BoundCalcDataService, BoundCalcLocation, MapItemLocationTab } from '../../boundcalc-data-service.service';
...
@Component({
  selector: 'app-boundcalc-loc-info-window',
  templateUrl: './boundcalc-loc-info-window.component.html',
  styleUrls: ['./boundcalc-loc-info-window.component.css']
})

export class BoundCalcLocInfoWindowComponent extends ComponentBase {
    constructor(
    ...
    loc: BoundCalcLocation| null = null
    private _nodes: Node[] = []
    ...
    deleteNode( e: IDeleteItem) {
        ...
        //??e.canDelete = this.boundcalcDataService.canDeleteNode(node)
    }

    editBranch(branch: Branch) {
        let branchEditorData = this.dataService.getBranchEditorData(branch.id)
        this.dialogService.showBoundCalcBranchDialog(branchEditorData)
    }
    ...
    addInternalBranch(e: any) {
        ...
        this.dialogService.showBoundCalcBranchDialog({branch: branchData, ctrl: undefined});
    }
