...
import { GoogleMap, MapAnchorPoint, MapEventManager } from '@angular/google-maps';
import { MAP_MARKER, MarkerDirective } from './marker-utilites';
...
export class MapAdvancedMarker
    implements OnInit, OnChanges, OnDestroy, MarkerDirective {
    ...
    ngOnInit() {
        ...
        if (google.maps.marker?.AdvancedMarkerElement && this._googleMap.googleMap) {
            ...
            this._ngZone.runOutsideAngular(() => {
                Promise.all([this._resolveMap(this._googleMap), google.maps.importLibrary('marker')]).then(
                    ([map, lib]) => {
    ...
    private _resolveMap(gMap: GoogleMap): Promise<google.maps.Map> {
        return gMap.googleMap
          ? Promise.resolve(gMap.googleMap)
          : gMap.mapInitialized.pipe(take(1)).toPromise();
    }
